...
//  SCIMBO
//
//  Created by Nirmal's Mac Mini on 02/07/19.
...
class SecondLoginVC: UIViewController,MICountryPickerDelegate,UITextFieldDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if UIDevice.isIphoneX {
            HeaderBarHeight.constant = Constant.sharedinstance.NavigationBarHeight_iPhoneX
        } else {
            HeaderBarHeight.constant = Constant.sharedinstance.NavigationBarHeight
        }
    ...
    func SetUI(){
        CancelView.dropShadow(shadowRadius: 8)
        ...
        CancelView.backgroundColor = customColor.themeColor
        NextView.backgroundColor = customColor.themeColor
        NextView.layer.cornerRadius = NextView.frame.height/2
       // HeaderLabel.text = "Enter your mobile number"
            ...
            if(error != nil)
            {
                ...
                if errNo != "1"{
                    ...
                    DispatchQueue.main.async {
                        self.moveHomeVC(Name: Name, mssidn_No: mssidn_No, User_id: user_id, imageUrl: image_Url, email: email)
                    }
                }
            }
        })
    ...
    @IBAction func didclickNextButton(_ sender: Any) {
        ...
        else if ((self.MobileTextField.text?.count)! <= 1 || (self.MobileTextField.text?.count)! >= 30) {
