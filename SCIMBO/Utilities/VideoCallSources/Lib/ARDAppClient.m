...
static NSString * const kARDIceServerRequestUrl = BASE_URL @"/iceserver";
...
- (void)connectToRoomWithId:(NSString *)roomId
    ...
    [self joinRoomServer:roomId isLoopback:isLoopback];
}

-(void)joinRoomServer:(NSString *)roomId isLoopback:(BOOL)isLoopback {
    __weak ARDAppClient *weakSelf = self;
...
- (void)receivedTurnMessage:(NSString *)payload {
    
    ARDSignalingMessage *message =
...
- (void)channel:(id<ARDSignalingChannel>)channel sendTurnMessage:(NSString *)message {
    [_delegate appClient:self sendTurnMessage:message];
}
...
- (void)sendSignalingMessage:(ARDSignalingMessage *)message {
    if (_isInitiator) {
        ...
        [_delegate appClient:self sendTurnMessagefromCaller:payload roomId:_roomId clientId:_clientId];
    } else {
...
- (void)configureAVAudioSession:(BOOL)earspeaker
{
    ...
    success = [session setCategory:AVAudioSessionCategoryPlayAndRecord mode:earspeaker ?  AVAudioSessionModeVoiceChat : AVAudioSessionModeVideoChat options:AVAudioSessionCategoryOptionDuckOthers error:&error];
    ...
    success = [session overrideOutputAudioPort: earspeaker ?  AVAudioSessionPortOverrideNone : AVAudioSessionPortOverrideSpeaker
                                         error:&error];
    if (!success) {
